...
// This file was generated by tars2go 1.1.9
...
// ShowGoReq is the proxy function for the method defined in the tars file, with the context
func (obj *SayHello) ShowGoReq(input string, output *string, opts ...map[string]string) (ret int32, err error) {
	var (
		length int32
		have   bool
		ty     byte
	...
	buf := codec.NewBuffer()
	err = buf.WriteString(input, 1)
	if err != nil {
		return ret, err
	}

	err = buf.WriteString(*output, 2)
	if err != nil {
		return ret, err
	}
	...
	err = obj.servant.TarsInvoke(tarsCtx, 0, "ShowGoReq", buf.ToBytes(), statusMap, contextMap, tarsResp)
	if err != nil {
		return ret, err
	}
	...
	err = readBuf.ReadString(&(*output), 2, true)
	if err != nil {
		return ret, err
	}
...
// ShowGoReqWithContext is the proxy function for the method defined in the tars file, with the context
func (obj *SayHello) ShowGoReqWithContext(tarsCtx context.Context, input string, output *string, opts ...map[string]string) (ret int32, err error) {
	var (
		length int32
		have   bool
		ty     byte
	...
	if ok && traceData.TraceCall {
		...
		tars.Trace(traceData.GetTraceKey(trace.EstCS), trace.TraceAnnotationCS, tars.GetClientConfig().ModuleName, obj.servant.Name(), "ShowGoReq", 0, traceParam, "")
	}
	...
	if ok && traceData.TraceCall {
		...
		tars.Trace(traceData.GetTraceKey(trace.EstCR), trace.TraceAnnotationCR, tars.GetClientConfig().ModuleName, obj.servant.Name(), "ShowGoReq", int(tarsResp.IRet), traceParam, "")
	}
...
// ShowGoReqOneWayWithContext is the proxy function for the method defined in the tars file, with the context
func (obj *SayHello) ShowGoReqOneWayWithContext(tarsCtx context.Context, input string, output *string, opts ...map[string]string) (ret int32, err error) {
	var (
		length int32
		have   bool
		ty     byte
	...
	tarsResp := new(requestf.ResponsePacket)
	err = obj.servant.TarsInvoke(tarsCtx, 1, "ShowGoReq", buf.ToBytes(), statusMap, contextMap, tarsResp)
	if err != nil {
		return ret, err
	}
...
type SayHelloServant interface {
	...
	ShowGoReq(input string, output *string) (ret int32, err error)
}
type SayHelloServantWithContext interface {
	...
	ShowGoReq(tarsCtx context.Context, input string, output *string) (ret int32, err error)
}
...
func (obj *SayHello) Dispatch(tarsCtx context.Context, val interface{}, tarsReq *requestf.RequestPacket, tarsResp *requestf.ResponsePacket, withContext bool) (err error) {
	...
	case "ShowGoReq":
		...
		if tarsReq.IVersion == basef.TARSVERSION {
			...
			err = readBuf.ReadString(&input, 1, true)
			if err != nil {
				return err
			}
			...
			reqTup.GetBuffer("input", &tupBuffer)
			readBuf.Reset(tupBuffer)
			err = readBuf.ReadString(&input, 0, true)
			if err != nil {
				return err
			}
			...
			{
				jsonStr, _ := json.Marshal(jsonData["input"])
		...
		if ok && traceData.TraceCall {
			...
			tars.Trace(traceData.GetTraceKey(trace.EstSR), trace.TraceAnnotationSR, tars.GetClientConfig().ModuleName, tarsReq.SServantName, "ShowGoReq", 0, traceParam, "")
		}

		var funRet int32
		if !withContext {
			imp := val.(SayHelloServant)
			funRet, err = imp.ShowGoReq(input, &output)
		} else {
			imp := val.(SayHelloServantWithContext)
			funRet, err = imp.ShowGoReq(tarsCtx, input, &output)
		}
		...
		if tarsReq.IVersion == basef.TARSVERSION {
			...
			buf.Reset()
			err = buf.WriteString(output, 0)
			if err != nil {
				return err
			}

			rspTup.PutBuffer("output", buf.ToBytes())
		...
		if ok && traceData.TraceCall {
			...
			tars.Trace(traceData.GetTraceKey(trace.EstSS), trace.TraceAnnotationSS, tars.GetClientConfig().ModuleName, tarsReq.SServantName, "ShowGoReq", 0, traceParam, "")
		}
